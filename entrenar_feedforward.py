#!/usr/bin/env python3
"""
Entrenamiento FeedForward para clasificación SMAW con YAMNet embeddings agregados.
...
def get_embeddings_cache_path(
    ...
    """Obtiene la ruta del archivo de cache de embeddings agregados."""
    cache_dir = get_embeddings_cache_dir(duration_dir)
    return (
        cache_dir / f"yamnet_embeddings_aggregated_{segment_duration}s_overlap_{overlap_ratio}.pkl"
    )
...
def get_legacy_embeddings_cache_paths(
    ...
    return [
        cache_dir / f"yamnet_embeddings_aggregated_{dur}s_overlap0.0s.pkl",
        cache_dir / f"yamnet_embeddings_aggregated_{dur}s_overlap_0.0.pkl",
        cache_dir / f"yamnet_embeddings_aggregated_{dur}s.pkl",
    ]
...
def main():
    ...
    # Extraer embeddings YAMNet agregados
    print("\nExtrayendo embeddings YAMNet agregados de todos los segmentos...")
    paths = all_data["Audio Path"].values
    segment_indices = all_data["Segment Index"].values

    # Intentar cargar desde cache
    all_features_list = None
    if not args.no_cache:
        all_features_list, cache_loaded = load_embeddings_cache(
            list(paths),
    ...
    sample_model = FeedForwardMultiTask(
        num_classes_espesor=len(np.unique(y_plate)),
        num_classes_electrodo=len(np.unique(y_electrode)),
        num_classes_corriente=len(np.unique(y_current)),
    )
